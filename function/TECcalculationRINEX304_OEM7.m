...
for i = 1: length(Sat_obs) % Satellite number (index)
    ...
    C1   = obs.data(Sat,ismember(obs.type,'C1C'));
        % Pseudorange P code      :L2   (m)
    P2 = obs.data(Sat,ismember(obs.type,'C2W'));
        % Carrier Phase in length :L1   (m)
    L1 = lambda1*obs.data(Sat,ismember(obs.type,'L1C'));
        % Carrier Phase in length :L2   (m)
    L2 = lambda2*obs.data(Sat,ismember(obs.type,'L2W'));
